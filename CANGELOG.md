...
- **表示切替をタブからリスト式セレクターへ刷新**
  - 初期表示を Claude Code + Codex の比較モードに変更
  - 比較モードでは checkbox で表示ソースを選択
  - 最低 1 件、最大 3 件の選択制約を追加
  - 表示切替時は再集計せず、ビュー表示時・セッション変更時・更新ボタン押下時のみ集計
  - 狭い幅でも比較表示を横並びで維持し、必要に応じて横スクロール

### Diff Review

- **未送信コメント行のハイライト表示を追加**
  - 保存済みかつ未送信のレビューコメントがある Diff 行を薄いピンク系の背景で表示
  - 単一行コメントと複数行範囲コメントの両方に対応
  - コメント送信または Single Task Runner 登録でコメントが削除されるとハイライトも解除

- **Diff コメントを Single Task Runner に登録できるように改善**
  - Single Task Runner MCP が有効なセッションで、Diff Review アクションバーに登録ボタンを表示
  - コメント 1 件につき Single Task Runner タスク 1 件として登録
  - 登録後は Single Task Runner 一覧を開き、既存の Start 操作で実行可能
  - 登録途中で失敗した場合は成功分だけコメントを削除し、未登録コメントは保持

### Single Task Runner

- **タスク一覧で `clear_before` 状態を確認・編集可能に改善**
  - editable なタスクは一覧上の checkbox で切替可能
  - 実行中など編集できないタスクでは checkbox を無効化

### Session Memo

- **右サイドバーに Session Memo ビューを追加**（`Ctrl+Shift+N`）
  - セッション単位のメモを右サイドバーで編集
  - アプリ起動中はフロントエンド側で draft を保持
  - 保存ボタンで `<session root>/.myT-x/session-memo.md` に永続化
  - セッション名変更時にも draft が失われにくいよう、セッションキー単位で管理
  - メモサイズは 1 MiB 上限でガード
  - 保存は一時ファイルから rename する atomic write 方式
