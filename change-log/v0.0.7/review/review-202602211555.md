...
**生成日時:** 2026-02-21 15:55
**統合元:**
- `review-20260221151449.md` — カテゴリ別包括的レビュー（subagent-app/router/session/sessionlog/frontend）
- `review-20260221153626.md` — 深掘り分析（Throttle欠落バグ / Mutex-Sync / Dedup Key）

**レビュー方針:** `review-pr-para.prompt.md` に準拠。バグ未然検知・パフォーマンス・設計方針の妥当性を重点的に調査。
...
#### A-0: `app:error-logged` イベントのThrottlingによるフロントエンドデータ欠落 ⚠️ 最重要

- **[`app_session_log.go` — `writeSessionLogEntry`]**
- **事象:** `now.Sub(a.sessionLogLastEmit) >= sessionLogEmitMinInterval (50ms)` の判定で、50ms以内に連続発生した `error` / `warn` レベルのエントリがイベント送信をスキップ (`shouldEmit = false`) される。
- **影響:** フロント (`useBackendSync.ts`) は `app:error-logged` イベントで受信したデータを Store に追記する設計のため、**間引かれたエラーがフロントエンドのメモリ上から完全に欠落**し、再ロードするまで画面に表示されない。
- **対応案 (推奨):**
  1. イベントに `entry` を乗せる仕様をやめ、「更新あり (`app:error-log-updated`)」という Ping のみを間引き送信し、フロント側で `GetSessionErrorLog()` をデバウンスフェッチする方式に変更。
  2. あるいは、スロットル期間中に溜め込んだログを配列として一括送信（バッチ処理）する方式に変更。

#### A-0b: Mutexロック下での同期的ファイル Sync によるストール

- **[`app_session_log.go` — `writeSessionLogEntry`]**
- **事象:** `a.sessionLogMu.Lock()` 保持中に `entry.Level == "error"` の場合 `a.sessionLogFile.Sync()` を呼ぶ（ディスクへ強制フラッシュ）。
- **影響:** ディスクI/O遅延が発生すると、本関数を呼び出すすべての `slog.Error()` とWailsログ監視全体がストールする。
- **対応案:** ファイルI/Oをチャネルを介した非同期ワーカーゴルーチンへ逃がす防御的設計を検討。
...
- `snapshotEventPolicies` の `NOTE(1-window model):` コメントに文字化け（`迴ｾ蝨ｨ縺ｮ...`形式）が発生。
- 「1セッション = 1ウィンドウ」設計方針の解説部分であり、開発者が誤読するリスクがある。
- **対処:** Shift-JIS/EUC-JP として誤認されているため、ファイル全体を UTF-8 として再保存。

#### A-2: セッションログファイル名の衝突リスク

- **[`app_session_log.go`:121付近]**
- セッションログファイルが `session-YYYYMMDD-HHMMSS.jsonl` 形式で命名されており、1秒以内の再起動時にファイル名が重複する可能性がある。
- **対処:** PID を付与（例: `session-YYYYMMDD-HHMMSS-<PID>.jsonl`）して一意性を保証。
...
- `tmux:window-created` の emit が削除されたことによるフロントエンド側への影響確認が必要。
- 既存セッション（複数ウィンドウ保持）が接続された場合の整合性再確認。

#### A-4: `slog.Default()` リストア漏れの可能性

- **[`app_lifecycle_test.go`]**
- テスト後に `slog.SetDefault()` でリストアするコードが漏れていないか確認。
- 他テストの実行順序依存で副作用が発生するリスク。
...
- **詳細デバッグログ:** `DevPanelGitStatus` でのdetached HEAD分岐に適切なデバッグログを追加。
...
#### B-1: `handleNewWindow` アーキテクチャ変更の整合性

- **[`command_router_handlers_window.go` 全体]**
- `handleNewWindow` が tmux 標準挙動 (`AddWindow`) を使わず、`CreateSession` による新セッション作成へ大幅変更。
- 「1セッション1ウィンドウ」モデルへの意図的変更だが、以下の確認が必要:
  1. `tmux:window-created` イベント削除によるクライアント側への影響
  2. 既存のマルチウィンドウを保持する古いセッションへの影響
  3. `AddWindow` API を呼んでいた他の経路（コードベース全体）の有無
...
- `new-window` は通常、親ペインの環境変数を継承すべきであり、意図的な場合はコメントで明示すべき。
...
- `boolPtr` ヘルパーがローカルに追加されているが、`command_router_env_test.go` では既に `testutil.Ptr` へリファクタリング済み。`boolPtr` を `testutil.Ptr[bool]` に統一すること。

#### B-5: `new-window` の `-n` フラグの必須化

- **[`command_router_handlers_window.go`:236-238]**
- 標準 tmux では `-n`（名前）は任意オプションだが、本実装では必須化している。tmux 互換性を意識する場合は自動名生成ロジック追加または明示的なドキュメント化が必要。

#### B-6: 削除テストケースのスキップ対応

- **[`command_router_handlers_window_test.go`]**
- マルチウィンドウ関連テストケースが削除されている。将来の再サポートに備えて `t.Skip("1-window model: multi-window not yet supported")` への切り替えを推奨。

### ✅ 良い点

- **13ステップコメント付きフロー:** `handleNewWindow` の処理フローが明確にドキュメント化されており保守性が高い。
- **ロールバック実装:** セッション作成後の失敗時に作成セッションを確実に削除するロールバックロジック。
...
- `AddWindow` メソッドが完全に削除されセッションに2番目以降のウィンドウを追加する公開APIが存在しなくなった。
- コンパイルエラーまたは機能欠落の可能性が極めて高い。
- **対処:** 1-windowモデルで不要であれば削除を明示的にドキュメント化し、将来のマルチウィンドウサポートのために存在を残すかどうかの設計判断を記録すること。
...
- 以下の詳細テストが削除され、簡略化された `TestRemoveWindowByID` にのみ置き換え:
  ...
  - `TestResolveDirectionalPane_MultipleWindowScopeIsolation` — ウィンドウ境界越え invariant 検証
...
- ペイン間の方向移動 (DirNext/DirPrev) がウィンドウ境界を越えないという Invariant が無検証。
...
- `RenameWindowByID` がイベント互換性のためにインデックスを返す仕様を維持しているが、単体でインデックスを取得する手段がなくなった。
...
- 引数名 `source` は `slog.Source`（ファイル名・行番号）と混同しやすい。`group` または `category` など意味的に明確な名前に変更を推奨。
...
- **[`internal/sessionlog/handler.go`:13-15]**
- 現在の `EntryCallback` はメッセージのみを受け取り `slog.Attr` を受け取れない。構造化ログの属性を通知・分析に活かすため、属性リストも引数に追加することを検討。

#### D-4: `WithAttrs` で付与された属性がcallback側で失われる

- **[`internal/sessionlog/handler.go`:67-73]**
- `WithAttrs` で指定された属性はベースハンドラには反映されるが、TeeHandler 自体が属性を保持しないため callback 呼び出し時には属性コンテキストが失われる。callback 側でも属性情報を利用したい場合は `TeeHandler` 構造体で属性を累積保持する必要がある。

#### D-5: ベースハンドラエラー時の挙動テストが未記述

- **[`internal/sessionlog/handler_test.go`]**
- ベースハンドラがエラーを返した場合の callback 呼び出し有無のテストが存在しない。意図した挙動を明文化するテストケースの追加を推奨。
...
> ✅ 追加調査により解消: `asObject` は `../utils/typeGuards` からインポートされており、`typeGuards.ts` ファイルの存在も確認済み。実行時エラーの懸念は解消。

### 🟡 重要な問題

#### E-1: エラーログ重複排除キー（Deduplication Key）の脆弱性

- **[`frontend/src/stores/errorLogStore.ts`]**
- バックエンドからのSnapshotデータと既存データのマージキーが `${e.ts}|${e.level}|${e.msg}|${e.source}` であり、バックエンドの `ts` が 14桁（秒単位精度）のため、**「同一秒」に発生した「同じ内容」の別エラーが誤って重複排除される**リスクがある。
- **対応案:** バックエンド (`SessionLogEntry`) 側で UUID またはインクリメントされる順序ID (`seq`) を持たせ、それをキーとする堅牢な仕様に変更（後述 Task-BE-2 と連動）。

#### E-2: \"c\" キーバインドの削除によるUX劣化

- **[`frontend/src/hooks/usePrefixKeyMode.ts`]**
- tmux 標準の \"c\" キー（`new-window`）のハンドラが完全削除。
- コメントには `NewSessionModal` を使うべきとあるが、ショートカットキーを完全無効化するのではなく、モーダルを開くアクションを割り当てる代替策を検討すること。
- **tmux互換性を重視するユーザーには重大な操作感の低下。**

#### E-3: Wailsjs API の削除（`AddWindow`, `RemoveWindow`, `RenameWindow`）

- **[`frontend/wailsjs/go/main/App.d.ts` / `.js`]**
- `AddWindow`, `RemoveWindow`, `RenameWindow` が削除されている。プロジェクト全体で他コンポーネントから参照されていないか確認が必要。

### 🔵 改善提案

#### E-4: `setEntries` の重複排除キー生成のパフォーマンス

- **[`frontend/src/stores/errorLogStore.ts`:38-42]**
- `deduplicationKey` 関数が全エントリを毎回走査して文字列キーを生成。`MAX_ENTRIES = 10,000` と大きく、頻繁な更新で影響が出る可能性あり。
...
#### E-5: `unreadCount` の非直感的リセット

- **[`frontend/src/stores/errorLogStore.ts`:45]**
- `setEntries`（バックエンドからの全取得時）で一律 `unreadCount = 0` にリセットする。ユーザーがエラーログ画面を開いていない状態でバックエンド同期が走った場合、未読数が消える。
- 画面表示状態（Visibility）と連動させるか、明示的な既読アクションを設けることを推奨。

#### E-6: CSS変数活用の統一

- **[`frontend/src/styles/viewer.css`]**
- `.error-log-view` 等の新設スタイルで色は CSS変数を使用しているが、フォントサイズや余白は直値になっている。テーマ対応やメンテナンス性向上のため変数化を推奨。
...
- **[`errorLogStore.ts`]:** ts+level+msg+source の複合キーによる高精度な重複排除（E-1 の改善で更に堅牢化可能）。
...
複数箇所で `NOTE(1-window model)` コメントとともに `else` ブランチや `tmux:window-*` イベントが「到達不能コード」として残されている。意図的な備えではあるが:
...
| ID | 重要度 | カテゴリ | 概要 |
|----|--------|---------|------|
| A-0 | 🔴 Critical | appレイヤー | Throttlingによるフロントエンドエラーログ欠落バグ |
| A-0b | 🔴 Critical | appレイヤー | Mutexロック下でのファイルSync — ストールリスク |
...
| E-1 | 🟡 Important | フロントエンド | Deduplication Keyの秒精度脆弱性 |
| E-2 | 🟡 Important | フロントエンド | \"c\" キーバインド削除によるUX劣化 |
| E-3 | 🟡 Important | フロントエンド | Wailsjs API 削除の参照漏れ確認 |
...
| E-4 | フロントエンド | 重複排除キー生成のパフォーマンス改善 |
| E-5 | フロントエンド | `unreadCount` リセットタイミングの改善 |
| E-6 | フロントエンド | CSS変数活用の統一 |
| X-1 | 横断 | 到達不能コードの文書化 |
...
> 🟢 = 並列作業可能 / 🟡 = 注意（依存あり） / 🔴 = 直列作業推奨（ブロッカー）

### Phase 1: ブロッカー解消（直列・最優先）

| タスクID | 重要度 | 作業内容 | 対象ファイル | 並列? | 依存タスク | 作業者へのコメント |
|---------|--------|---------|------------|------|-----------|----------------|
| **A-0** | 🔴 Critical | Throttle欠落バグ修正<br>Ping通知+フェッチ方式 or 配列一括送信 | `app_session_log.go`<br>`app_session_log_types.go` | 🔴 | — | **最重要。他Eタスクが依存。仕様（Payload定義）を先に確定させること** |
| **A-0b** | 🔴 Critical | Mutexロック内Sync非同期化 | `app_session_log.go` | 🔴 | — | A-0と同一ファイル。A-0担当者が一緒に対応するのが安全 |
| **C-1** | 🔴 Critical | `AddWindow` 設計判断と復元/削除の明示 | `session_manager_windows.go` | 🔴 | — | 他のタスクが依存する基盤。先に設計方針を確定させること |
| **B-1** | 🔴 Critical | `handleNewWindow` アーキテクチャ変更の影響調査 | `command_router_handlers_window.go`<br>フロントエンド全体 | 🔴 | C-1 | C-1 と同時に設計確認。FE/BEの両エキスパートが連携して確認 |

### Phase 2: 独立タスク（完全並列）

| タスクID | 重要度 | 作業内容 | 対象ファイル | 並列? | 依存タスク | 作業者へのコメント |
|---------|--------|---------|------------|------|-----------|----------------|
| **A-1** | 🟡 | コメント文字化け修正 | `app_events.go` | 🟢 | — | 単純なテキスト修正。完全独立 |
| **A-2** | 🟡 | ログファイル名にPID付与 | `app_session_log.go` | 🟢 | — | 命名規則のみ。他への影響なし |
| **B-2** | 🟡 | CreateSession直後スナップショット失敗時ロールバック | `command_router_handlers_window.go` | 🟢 | — | B-1調査結果に依らず修正可能 |
| **B-3** | 🟡 | `nil inheritedEnv` への意図明示コメント追加 | `command_router_handlers_window.go` | 🟢 | — | コメント追加のみ |
...
| **C-3** | 🟡 | 方向ナビゲーション隔離テスト復元 | `session_manager_directional_test.go` | 🟢 | — | このファイルのみに限定 |
| **C-4** | 🟡 | `@stableID` 検証テスト復元 | `session_manager_pane_io_test.go` | 🟢 | — | このファイルのみに限定 |
| **C-5** | 🟡 | `ResolveTarget` 修復ロジックテスト復元 | `session_manager_test.go` | 🟢 | — | このファイルのみ |
| **D-1** | 🟡 | ベースハンドラエラー時のcallback実行設計変更 | `internal/sessionlog/handler.go` | 🟢 | — | 独立したパッケージ。完全並列可能 |
| **E-1** | 🟡 | Dedup Keyを秒単位→BackendID方式へ変更 | `frontend/src/stores/errorLogStore.ts` | 🟡 | A-0<br>（BE側ID実装後） | BEでID実装さえ合意・完了すれば独立して改修可能 |
| **E-3** | 🟡 | Wailsjs 削除 API 参照確認 | プロジェクト全体 (grep) | 🟢 | — | 調査のみ。コード変更不要の場合も多い |
| **E-4** | 💡 | `setEntries` パフォーマンス改善（Map活用） | `frontend/src/stores/errorLogStore.ts` | 🟢 | — | 独立したストア。完全並列可能 |
| **X-1** | 💡 | 到達不能コードのコメント文書化 | `app_events.go`<br>`command_router_handlers_window.go` | 🟢 | — | コメント追加のみ |
| **X-2** | 💡 | `.gitattributes` CRLF/LF 設定 | `.gitattributes` | 🟢 | — | 完全独立。即座に作業可能 |
| **X-3** | 💡 | テスト削除 → `t.Skip` 一括変換 | 全 `*_test.go` | 🟢 | — | テキスト変換のみ。機能影響なし |

### Phase 3: Phase1/2 依存解消後

| タスクID | 重要度 | 作業内容 | 対象ファイル | 並列? | 依存タスク | 作業者へのコメント |
|---------|--------|---------|------------|------|-----------|----------------|
| **C-2** | 🔴 | 削除されたエッジケーステストの復元 | `session_manager_window_test.go` | 🟡 | C-1完了後 | テスト追加のみなので他への影響なし |
| **C-7** | 💡 | `TestRemoveWindowByID` エッジケース拡充 | `session_manager_window_test.go` | 🟡 | C-2と同一ファイル。同一担当者推奨 | C-2と併せて対応 |
| **C-8** | 💡 | Terminal オブジェクトの具体的検証 | `session_manager_window_test.go` | 🟡 | C-2と同一ファイル。同一担当者推奨 | C-2と併せて対応 |
| **D-2** | 💡 | `source` → `group` 引数名変更 | `internal/sessionlog/handler.go`<br>`handler_test.go` | 🟡 | D-1確定後 | D-1と同一ファイル。同一担当者に割り当て推奨 |
| **D-3** | 💡 | `EntryCallback` に属性情報追加 | `internal/sessionlog/handler.go` | 🟡 | D-1確定後 | D-1と並行可能だが方針確定後が安全 |
| **D-4** | 💡 | `WithAttrs` 属性のcallback伝播 | `internal/sessionlog/handler.go` | 🟡 | D-1,D-3確定後 | 設計依存大きい |
| **D-5** | 💡 | エラー時挙動のテスト追加 | `internal/sessionlog/handler_test.go` | 🟡 | D-1設計確定後 | 先にD-1のポリシーを決めること |
| **E-2** | 🟡 | \"c\" キーバインドの代替アクション実装 | `frontend/src/hooks/usePrefixKeyMode.ts` | 🟡 | UI合意後 | NewSessionModal の使い方の合意が先 |
| **E-5** | 💡 | `unreadCount` リセット改善 | `frontend/src/stores/errorLogStore.ts` | 🟡 | E-4と同一ファイル。同一担当者推奨 | E-4と併せて対応推奨 |
| **E-6** | 💡 | CSS変数活用の統一 | `frontend/src/styles/viewer.css` | 🟢 | — | 独立作業可能 |
| **A-3** | 💡 | 1-windowモデル変更副作用確認 | `app_events.go` | 🟡 | B-1確認後 | B-1調査結果を参照するため確認後に実施 |
| **A-4** | 💡 | `slog.Default()` リストア漏れ確認 | `app_lifecycle_test.go` | 🟢 | — | 独立確認作業 |

---

## 推奨アクション順序（サブエージェント割り当て）

```
Phase 1 (ブロッカー解消 — 直列):
  └─ BE担当: A-0, A-0b (仕様を先にFEと合意!)
  └─ BE担当: C-1 → B-1（C-1設計確定後）

Phase 2 (独立タスク — 完全並列):
  ├─ 担当者A (appレイヤー):   A-1, A-2, X-1, X-2
  ├─ 担当者B (コマンドルーター): B-2, B-3, B-4, B-6
  ├─ 担当者C (セッションマネージャー): C-3, C-4, C-5, X-3
  ├─ 担当者D (セッションログ):  D-1 → (確定後) D-2, D-5
  └─ 担当者E (フロントエンド):  E-3, E-4, E-6（, E-1はA-0完了後）

Phase 3 (依存解消後):
  ├─ C-1完了後: C-2, C-7, C-8
  ├─ D-1完了後: D-2, D-3, D-4, D-5
  ├─ A-0完了後(BE/FE合意後): E-1 (Dedup Key刷新)
  └─ UI合意後: E-2 (\"c\" キーバインド代替実装)
```

> ※ A-0のPayload定義（新しいイベント名・データ構造・識別子IDルール）のみ、FE/BE担当者で事前合意の上、Phase 2を並列実行することで全体工期を最短化できます。
