...
myT-x 内蔵のオーケストレーション MCP が提供する全 19 ツールの詳細仕様です。
...
- `add_members` で複数メンバーを一括追加できる。二段階アプローチにより `add_member` の繰り返し呼び出しより高速
...
複数のメンバーを一括追加する（`add_member` のバッチ版）。二段階アプローチにより効率的にチームを構築する。

- Phase 1: ペイン分割 + CLI 起動を高速に連続実行
- Phase 2: 全 CLI 起動後に1回だけ待ち → ブートストラップメッセージを順次送信

**実行権限:** 登録済みエージェントのみ

| パラメータ | 必須 | 型 | 説明 |
|-----------|------|-----|------|
| `members` | Yes | array | メンバー定義配列（1〜10 件） |
| `team_name` | No | string | チーム名（最大 64 文字、デフォルト: `動的チーム`） |
| `split_from` | No | string | 最初の分割元ペイン ID（デフォルト: 呼び出し元ペイン） |
| `split_direction` | No | string | `horizontal` / `vertical`（デフォルト: `horizontal`） |
| `bootstrap_delay_ms` | No | integer | 全 CLI 起動後の待ち時間 ms（1000〜30000、デフォルト: 3000） |
...
**戻り値:** `results[]`（各メンバーの `pane_id`, `pane_title`, `agent_name`, `warnings`, `error`）, `summary` { `created`, `failed` }

**注意事項:**
- `add_member` のバッチ版。二段階実行により所要時間を大幅に短縮する
- 各メンバーは直前に作成されたペインから分割される（カスケード配置）
- `split_from` 省略時は、最初のメンバーは呼び出し元ペインから分割する
- 個別メンバーの失敗はバッチ全体を停止しない。結果に `error` が記録される
- Claude CLI（claude, claude.exe, claude-code*）はブラケットペーストモードで送信される
- ペインタイトル設定やブートストラップ送信の失敗は `warnings` として返される
