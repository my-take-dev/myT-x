...
export function QuickSearch({ open, onClose }: QuickSearchProps) {
  ...
  const selectResult = (sessionName: string) => {
    setActiveSession(sessionName);
    void api.SetActiveSession(sessionName).catch((err) => {
  ...
  const handleKeyDown = (e: React.KeyboardEvent) => {
    ...
    if (e.key === "Enter" && results[selectedIndex]) {
      selectResult(results[selectedIndex].sessionName);
    }
  };
        ...
        <div className="quick-search-results" id={listboxId} role="listbox" aria-label="検索結果">
          {results.map((r, i) => (
            <div
              ...
              onClick={() => selectResult(r.sessionName)}
