...
import type {OrchestratorTeamDraft} from "./types";
import {
...
export function TeamEditor({
    ...
    const {t} = useI18n();
    const bootstrapDelay = useBootstrapDelayInput({sourceMs: draft.bootstrapDelayMs});

    useEffect(() => {
        const normalizedBootstrapDelayMs = bootstrapDelaySecToMs(formatBootstrapDelaySec(draft.bootstrapDelayMs));
        if (draft.bootstrapDelayMs !== normalizedBootstrapDelayMs) {
            onChange({...draft, bootstrapDelayMs: normalizedBootstrapDelayMs});
        }
    }, [draft, onChange]);

    const handleBootstrapDelayChange = useCallback((event: ChangeEvent<HTMLInputElement>) => {
        bootstrapDelay.handleChange(event);
        const normalizedValue = normalizeBootstrapDelaySecInput(event.target.value);
        if (normalizedValue === null) {
            return;
        }
        onChange({...draft, bootstrapDelayMs: bootstrapDelaySecToMs(normalizedValue)});
    }, [bootstrapDelay, draft, onChange]);
