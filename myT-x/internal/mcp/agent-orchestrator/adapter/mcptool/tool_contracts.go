...
var (
	sendTaskHelpContract = helpToolContract{
		registryDescription: "Send a task to a target agent. Caller-pane registration is optional, but from_agent must resolve to a registered agent so the assignee can reply via send_response. Inline delivery is used up to 16000 characters; larger payloads spill to files instead of pasting the full body.",
		helpDescription:     "Send a task to a target agent. A successful call returns task_id, and stored-payload metadata can be inspected later through get_task_message or get_task_detail.",
		notes: []string{
			...
			"send_task itself returns task and pane identifiers only; use get_task_message or get_task_detail to inspect spillover metadata.",
			"By default, response instructions are appended to the end of the outgoing message before delivery and persistence.",
	...
	sendTasksHelpContract = helpToolContract{
		registryDescription: "Batch-send tasks to multiple agents, grouped by group_id. Requires agent registration or trusted access. Each task item accepts agent_name, message, include_response_instructions, and expires_after_minutes. Inline delivery is used up to 16000 characters; larger payloads spill to files.",
