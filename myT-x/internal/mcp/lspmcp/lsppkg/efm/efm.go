// Package efm は EFM 向けの MCP 拡張ツールを提供する。
package efm
...
const efmReferenceURL = "https://github.com/mattn/efm-langserver"

// BuildTools provides EFM language-server specific extension tools.
func BuildTools(client *lsp.Client, rootDir string) []mcp.Tool {
	...
	return []mcp.Tool{
		{
			Name:        "efm_list_extension_commands",
			Description: triadDescription("Inspect workspace commands advertised via executeCommandProvider.commands by the connected EFM language server", "none", "read"),
			InputSchema: emptySchema(),
			Handler:     svc.handleListCommands,
		},
		{
			Name:        "efm_execute_extension_command",
			Description: triadDescription("Run one workspace command advertised by the connected EFM language server", "command, arguments?", "exec"),
...
// Matches returns true when the configured LSP command/args indicate EFM language server.
func Matches(command string, args []string) bool {
	if looksLikeEFMServer(command) {
		return true
	}

	if slices.ContainsFunc(args, looksLikeEFMServer) {
		return true
	}

	if looksLikeGo(command) && referencesEFMServer(args) {
		return true
	}
...
func (s *service) handleListCommands(_ context.Context, _ map[string]any) (any, error) {
	...
	for _, name := range commands {
		when, argsHint, effect := capabilityCommandGuide(name, "EFM")
	...
	return map[string]any{
		...
		"count":             len(commands),
		"references":        []string{efmReferenceURL},
	}, nil
}
...
func (s *service) handleExecuteCommand(ctx context.Context, args map[string]any) (any, error) {
	...
	when, argsHint, effect := capabilityCommandGuide(command, "EFM")
...
func looksLikeEFMServer(value string) bool {
	base := strings.ToLower(strings.TrimSpace(filepath.Base(value)))
	switch base {
	case "efm-langserver", "efm-langserver.exe":
		return true
	default:
		return strings.Contains(base, "efm-langserver")
	}
}
...
func referencesEFMServer(args []string) bool {
	return slices.ContainsFunc(args, func(arg string) bool {
		normalized := strings.ToLower(strings.TrimSpace(arg))
		return strings.Contains(normalized, "efm-langserver") ||
			strings.Contains(normalized, "github.com/mattn/efm-langserver")
	})
}
	...
	return map[string]any{
		...
		"properties": map[string]any{
			"command": map[string]any{
				"type":        "string",
				"description": "EFM language-server command name to execute (see efm_list_extension_commands).",
			},
