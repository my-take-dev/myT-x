// Package erg は Erg 向けの MCP 拡張ツールを提供する。
package erg
...
const ergReferenceURL = "https://github.com/erg-lang/erg"

// BuildTools は Erg 言語サーバー向けの拡張ツールを構築する。
func BuildTools(client *lsp.Client, rootDir string) []mcp.Tool {
	...
	return []mcp.Tool{
		{
			Name:        "erg_list_extension_commands",
			Description: triadDescription("Inspect workspace commands advertised via executeCommandProvider.commands by the connected Erg language server", "none", "read"),
			InputSchema: emptySchema(),
			Handler:     svc.handleListCommands,
		},
		{
			Name:        "erg_execute_extension_command",
			Description: triadDescription("Run one workspace command advertised by the connected Erg language server", "command, arguments?", "exec"),
...
// Matches は設定されたコマンド/引数が Erg 言語サーバーを示す場合に true を返す。
func Matches(command string, args []string) bool {
	if looksLikeErgServer(command) {
		return true
	}

	if slices.ContainsFunc(args, looksLikeErgServer) {
		return true
	}

	if looksLikeErgRunner(command) && referencesErgServer(args) {
		return true
	}
...
func (s *service) handleListCommands(_ context.Context, _ map[string]any) (any, error) {
	...
	for _, name := range commands {
		when, argsHint, effect := capabilityCommandGuide(name, "Erg")
	...
	return map[string]any{
		...
		"count":             len(commands),
		"references":        []string{ergReferenceURL},
	}, nil
}
...
func (s *service) handleExecuteCommand(ctx context.Context, args map[string]any) (any, error) {
	...
	when, argsHint, effect := capabilityCommandGuide(command, "Erg")
...
func looksLikeErgServer(value string) bool {
	base := strings.ToLower(strings.TrimSpace(filepath.Base(value)))
	switch base {
	case "els", "els.exe",
		"erg-language-server", "erg-language-server.exe":
		return true
	default:
		return strings.Contains(base, "erg-language-server")
	}
}

func looksLikeErgRunner(value string) bool {
	base := strings.ToLower(strings.TrimSpace(filepath.Base(value)))
	return base == "erg" || base == "erg.exe"
}

func referencesErgServer(args []string) bool {
	return slices.ContainsFunc(args, func(arg string) bool {
		normalized := strings.ToLower(strings.TrimSpace(arg))
		return looksLikeErgServer(normalized) ||
			normalized == "lsp" ||
			normalized == "--language-server" ||
			normalized == "--lsp" ||
			strings.Contains(normalized, "erg-language-server")
	})
}
	...
	return map[string]any{
		...
		"properties": map[string]any{
			"command": map[string]any{
				"type":        "string",
				"description": "Erg language-server command name to execute (see erg_list_extension_commands).",
			},
