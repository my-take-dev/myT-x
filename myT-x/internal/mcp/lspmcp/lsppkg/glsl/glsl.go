// Package glsl は GLSL 向けの MCP 拡張ツールを提供する。
package glsl
...
const glslReferenceURL = "https://github.com/svenstaro/glsl-language-server"

// BuildTools は GLSL 言語サーバー向けの拡張ツールを構築する。
func BuildTools(client *lsp.Client, rootDir string) []mcp.Tool {
	...
	return []mcp.Tool{
		{
			Name:        "glsl_list_extension_commands",
			Description: triadDescription("Inspect workspace commands advertised via executeCommandProvider.commands by the connected GLSL language server", "none", "read"),
			InputSchema: emptySchema(),
			Handler:     svc.handleListCommands,
		},
		{
			Name:        "glsl_execute_extension_command",
			Description: triadDescription("Run one workspace command advertised by the connected GLSL language server", "command, arguments?", "exec"),
...
// Matches は設定されたコマンド/引数が GLSL 言語サーバーを示す場合に true を返す。
func Matches(command string, args []string) bool {
	if looksLikeGLSLServer(command) {
		return true
	}

	if slices.ContainsFunc(args, looksLikeGLSLServer) {
		return true
	}
...
func (s *service) handleListCommands(_ context.Context, _ map[string]any) (any, error) {
	...
	for _, name := range commands {
		when, argsHint, effect := capabilityCommandGuide(name, "GLSL")
	...
	return map[string]any{
		...
		"count":             len(commands),
		"references":        []string{glslReferenceURL},
	}, nil
}
...
func (s *service) handleExecuteCommand(ctx context.Context, args map[string]any) (any, error) {
	...
	when, argsHint, effect := capabilityCommandGuide(command, "GLSL")
...
func looksLikeGLSLServer(value string) bool {
	base := strings.ToLower(strings.TrimSpace(filepath.Base(value)))
	switch base {
	case "glsl-language-server", "glsl-language-server.exe",
		"glsl-lsp", "glsl-lsp.exe",
		"glslls", "glslls.exe":
		return true
	default:
		return strings.Contains(base, "glsl-language-server") ||
			strings.Contains(base, "glsl-lsp") ||
			strings.Contains(base, "glslls")
	}
}
	...
	return map[string]any{
		...
		"properties": map[string]any{
			"command": map[string]any{
				"type":        "string",
				"description": "GLSL language-server command name to execute (see glsl_list_extension_commands).",
			},
