// Package gn は GN 向けの MCP 拡張ツールを提供する。
package gn
...
const gnReferenceURL = "https://github.com/google/gn-language-server"

// BuildTools は GN 言語サーバー向けの拡張ツールを構築する。
func BuildTools(client *lsp.Client, rootDir string) []mcp.Tool {
	...
	return []mcp.Tool{
		{
			Name:        "gn_list_extension_commands",
			Description: triadDescription("Inspect workspace commands advertised via executeCommandProvider.commands by the connected GN language server", "none", "read"),
			InputSchema: emptySchema(),
			Handler:     svc.handleListCommands,
		},
		{
			Name:        "gn_execute_extension_command",
			Description: triadDescription("Run one workspace command advertised by the connected GN language server", "command, arguments?", "exec"),
...
// Matches は設定されたコマンド/引数が GN 言語サーバーを示す場合に true を返す。
func Matches(command string, args []string) bool {
	if looksLikeGNServer(command) {
		return true
	}

	if slices.ContainsFunc(args, looksLikeGNServer) {
		return true
	}

	if looksLikeCargo(command) && referencesGNServer(args) {
		return true
	}
...
func (s *service) handleListCommands(_ context.Context, _ map[string]any) (any, error) {
	...
	for _, name := range commands {
		when, argsHint, effect := capabilityCommandGuide(name, "GN")
	...
	return map[string]any{
		...
		"count":             len(commands),
		"references":        []string{gnReferenceURL},
	}, nil
}
...
func (s *service) handleExecuteCommand(ctx context.Context, args map[string]any) (any, error) {
	...
	when, argsHint, effect := capabilityCommandGuide(command, "GN")
...
func looksLikeGNServer(value string) bool {
	base := strings.ToLower(strings.TrimSpace(filepath.Base(value)))
	switch base {
	case "gn-language-server", "gn-language-server.exe",
		"gnls", "gnls.exe":
		return true
	default:
		return strings.Contains(base, "gn-language-server") || strings.Contains(base, "gnls")
	}
}
...
func referencesGNServer(args []string) bool {
	return slices.ContainsFunc(args, func(arg string) bool {
		normalized := strings.ToLower(strings.TrimSpace(arg))
		return strings.Contains(normalized, "gn-language-server") ||
			strings.Contains(normalized, "google/gn-language-server") ||
			strings.Contains(normalized, "gnls")
	})
}
	...
	return map[string]any{
		...
		"properties": map[string]any{
			"command": map[string]any{
				"type":        "string",
				"description": "GN language-server command name to execute (see gn_list_extension_commands).",
			},
