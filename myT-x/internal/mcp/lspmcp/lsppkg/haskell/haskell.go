// Package haskell は Haskell 向けの MCP 拡張ツールを提供する。
package haskell
...
const haskellReferenceURL = "https://github.com/haskell/haskell-language-server"

// BuildTools は Haskell 言語サーバー向けの拡張ツールを構築する。
func BuildTools(client *lsp.Client, rootDir string) []mcp.Tool {
	...
	return []mcp.Tool{
		{
			Name:        "haskell_list_extension_commands",
			Description: triadDescription("Inspect workspace commands advertised via executeCommandProvider.commands by the connected Haskell language server", "none", "read"),
			InputSchema: emptySchema(),
			Handler:     svc.handleListCommands,
		},
		{
			Name:        "haskell_execute_extension_command",
			Description: triadDescription("Run one workspace command advertised by the connected Haskell language server", "command, arguments?", "exec"),
...
// Matches は設定されたコマンド/引数が Haskell 言語サーバーを示す場合に true を返す。
func Matches(command string, args []string) bool {
	if looksLikeHaskellServer(command) {
		return true
	}

	if slices.ContainsFunc(args, looksLikeHaskellServer) {
		return true
	}
...
func (s *service) handleListCommands(_ context.Context, _ map[string]any) (any, error) {
	...
	for _, name := range commands {
		when, argsHint, effect := capabilityCommandGuide(name, "Haskell")
	...
	return map[string]any{
		...
		"count":             len(commands),
		"references":        []string{haskellReferenceURL},
	}, nil
}
...
func (s *service) handleExecuteCommand(ctx context.Context, args map[string]any) (any, error) {
	...
	when, argsHint, effect := capabilityCommandGuide(command, "Haskell")
...
func looksLikeHaskellServer(value string) bool {
	base := strings.ToLower(strings.TrimSpace(filepath.Base(value)))
	switch base {
	case "haskell-language-server", "haskell-language-server.exe",
		"haskell-language-server-wrapper", "haskell-language-server-wrapper.exe",
		"hls", "hls.exe":
		return true
	default:
		return strings.Contains(base, "haskell-language-server")
	}
}
	...
	return map[string]any{
		...
		"properties": map[string]any{
			"command": map[string]any{
				"type":        "string",
				"description": "Haskell language-server command name to execute (see haskell_list_extension_commands).",
			},
