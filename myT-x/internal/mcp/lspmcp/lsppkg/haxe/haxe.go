// Package haxe は Haxe 向けの MCP 拡張ツールを提供する。
package haxe
...
const haxeReferenceURL = "https://github.com/vshaxe/haxe-language-server"

// BuildTools は Haxe 言語サーバー向けの拡張ツールを構築する。
func BuildTools(client *lsp.Client, rootDir string) []mcp.Tool {
	...
	return []mcp.Tool{
		{
			Name:        "haxe_list_extension_commands",
			Description: triadDescription("Inspect workspace commands advertised via executeCommandProvider.commands by the connected Haxe language server", "none", "read"),
			InputSchema: emptySchema(),
			Handler:     svc.handleListCommands,
		},
		{
			Name:        "haxe_execute_extension_command",
			Description: triadDescription("Run one workspace command advertised by the connected Haxe language server", "command, arguments?", "exec"),
...
// Matches は設定されたコマンド/引数が Haxe 言語サーバーを示す場合に true を返す。
func Matches(command string, args []string) bool {
	if looksLikeHaxeServer(command) {
		return true
	}

	if slices.ContainsFunc(args, looksLikeHaxeServer) {
		return true
	}

	if looksLikeNode(command) && referencesHaxeServer(args) {
		return true
	}
...
func (s *service) handleListCommands(_ context.Context, _ map[string]any) (any, error) {
	...
	for _, name := range commands {
		when, argsHint, effect := capabilityCommandGuide(name, "Haxe")
	...
	return map[string]any{
		...
		"count":             len(commands),
		"references":        []string{haxeReferenceURL},
	}, nil
}
...
func (s *service) handleExecuteCommand(ctx context.Context, args map[string]any) (any, error) {
	...
	when, argsHint, effect := capabilityCommandGuide(command, "Haxe")
...
func looksLikeHaxeServer(value string) bool {
	base := strings.ToLower(strings.TrimSpace(filepath.Base(value)))
	switch base {
	case "haxe-language-server", "haxe-language-server.cmd", "haxe-language-server.exe":
		return true
	default:
		return strings.Contains(base, "haxe-language-server")
	}
}
...
func referencesHaxeServer(args []string) bool {
	return slices.ContainsFunc(args, func(arg string) bool {
		normalized := strings.ToLower(strings.TrimSpace(arg))
		return strings.Contains(normalized, "haxe-language-server")
	})
}
	...
	return map[string]any{
		...
		"properties": map[string]any{
			"command": map[string]any{
				"type":        "string",
				"description": "Haxe language-server command name to execute (see haxe_list_extension_commands).",
			},
