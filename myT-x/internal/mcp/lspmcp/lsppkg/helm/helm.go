// Package helm は Helm 向けの MCP 拡張ツールを提供する。
package helm
...
const helmReferenceURL = "https://github.com/mrjosh/helm-ls"

// BuildTools は Helm 言語サーバー向けの拡張ツールを構築する。
func BuildTools(client *lsp.Client, rootDir string) []mcp.Tool {
	...
	return []mcp.Tool{
		{
			Name:        "helm_list_extension_commands",
			Description: triadDescription("Inspect workspace commands advertised via executeCommandProvider.commands by the connected Helm language server", "none", "read"),
			InputSchema: emptySchema(),
			Handler:     svc.handleListCommands,
		},
		{
			Name:        "helm_execute_extension_command",
			Description: triadDescription("Run one workspace command advertised by the connected Helm language server", "command, arguments?", "exec"),
...
// Matches は設定されたコマンド/引数が Helm 言語サーバーを示す場合に true を返す。
func Matches(command string, args []string) bool {
	if looksLikeHelmServer(command) {
		return true
	}

	if slices.ContainsFunc(args, looksLikeHelmServer) {
		return true
	}
...
func (s *service) handleListCommands(_ context.Context, _ map[string]any) (any, error) {
	...
	for _, name := range commands {
		when, argsHint, effect := capabilityCommandGuide(name, "Helm")
	...
	return map[string]any{
		...
		"count":             len(commands),
		"references":        []string{helmReferenceURL},
	}, nil
}
...
func (s *service) handleExecuteCommand(ctx context.Context, args map[string]any) (any, error) {
	...
	when, argsHint, effect := capabilityCommandGuide(command, "Helm")
...
func looksLikeHelmServer(value string) bool {
	base := strings.ToLower(strings.TrimSpace(filepath.Base(value)))
	switch base {
	case "helm-ls", "helm-ls.exe",
		"helm_ls", "helm_ls.exe":
		return true
	default:
		return strings.Contains(base, "helm-ls") || strings.Contains(base, "helm_ls")
	}
}
	...
	return map[string]any{
		...
		"properties": map[string]any{
			"command": map[string]any{
				"type":        "string",
				"description": "Helm language-server command name to execute (see helm_list_extension_commands).",
			},
