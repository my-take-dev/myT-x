// Package javascript は JavaScript 向けの MCP 拡張ツールを提供する。
package javascript
...
const javascriptReferenceURL = "https://github.com/quick-lint/quick-lint-js"

// BuildTools は JavaScript 言語サーバー向けの拡張ツールを構築する。
func BuildTools(client *lsp.Client, rootDir string) []mcp.Tool {
	...
	return []mcp.Tool{
		{
			Name:        "javascript_list_extension_commands",
			Description: triadDescription("Inspect workspace commands advertised via executeCommandProvider.commands by the connected JavaScript language server", "none", "read"),
			InputSchema: emptySchema(),
			Handler:     svc.handleListCommands,
		},
		{
			Name:        "javascript_execute_extension_command",
			Description: triadDescription("Run one workspace command advertised by the connected JavaScript language server", "command, arguments?", "exec"),
...
// Matches は設定されたコマンド/引数が quick-lint-js を示す場合に true を返す。
func Matches(command string, args []string) bool {
	if looksLikeJavaScriptServer(command) {
		return true
	}

	if slices.ContainsFunc(args, looksLikeJavaScriptServer) {
		return true
	}
...
func (s *service) handleListCommands(_ context.Context, _ map[string]any) (any, error) {
	...
	for _, name := range commands {
		when, argsHint, effect := capabilityCommandGuide(name, "JavaScript")
	...
	return map[string]any{
		...
		"count":             len(commands),
		"references":        []string{javascriptReferenceURL},
	}, nil
}
...
func (s *service) handleExecuteCommand(ctx context.Context, args map[string]any) (any, error) {
	...
	when, argsHint, effect := capabilityCommandGuide(command, "JavaScript")
...
func looksLikeJavaScriptServer(value string) bool {
	base := strings.ToLower(strings.TrimSpace(filepath.Base(value)))
	switch base {
	case "quick-lint-js", "quick-lint-js.exe", "quick-lint-js.cmd",
		"quick-lint-js-lsp", "quick-lint-js-lsp.exe":
		return true
	default:
		return strings.Contains(base, "quick-lint-js")
	}
}
	...
	return map[string]any{
		...
		"properties": map[string]any{
			"command": map[string]any{
				"type":        "string",
				"description": "JavaScript language-server command name to execute (see javascript_list_extension_commands).",
			},
