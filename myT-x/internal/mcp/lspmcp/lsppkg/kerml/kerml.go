// Package kerml は KerML 向けの MCP 拡張ツールを提供する。
package kerml
...
const kermlReferenceURL = "https://github.com/Sensmetry/sysml2-tools"

// BuildTools は KerML 言語サーバー向けの拡張ツールを構築する。
func BuildTools(client *lsp.Client, rootDir string) []mcp.Tool {
	...
	return []mcp.Tool{
		{
			Name:        "kerml_list_extension_commands",
			Description: triadDescription("Inspect workspace commands advertised via executeCommandProvider.commands by the connected KerML language server", "none", "read"),
			InputSchema: emptySchema(),
			Handler:     svc.handleListCommands,
		},
		{
			Name:        "kerml_execute_extension_command",
			Description: triadDescription("Run one workspace command advertised by the connected KerML language server", "command, arguments?", "exec"),
...
// Matches は設定されたコマンド/引数が KerML 言語サーバーを示す場合に true を返す。
func Matches(command string, args []string) bool {
	if looksLikeKerMLServer(command) {
		return true
	}

	if slices.ContainsFunc(args, looksLikeKerMLServer) {
		return true
	}

	if looksLikeNode(command) && referencesKerMLServer(args) {
		return true
	}

	if looksLikeNodeRunner(command) && referencesKerMLServer(args) {
		return true
	}
...
func (s *service) handleListCommands(_ context.Context, _ map[string]any) (any, error) {
	...
	for _, name := range commands {
		when, argsHint, effect := capabilityCommandGuide(name, "KerML")
	...
	return map[string]any{
		...
		"count":             len(commands),
		"references":        []string{kermlReferenceURL},
	}, nil
}
...
func (s *service) handleExecuteCommand(ctx context.Context, args map[string]any) (any, error) {
	...
	when, argsHint, effect := capabilityCommandGuide(command, "KerML")
...
func looksLikeKerMLServer(value string) bool {
	base := strings.ToLower(strings.TrimSpace(filepath.Base(value)))
	switch base {
	case "kerml-language-server", "kerml-language-server.cmd", "kerml-language-server.exe",
		"kerml-lsp", "kerml-lsp.cmd", "kerml-lsp.exe",
		"sysml2-language-server", "sysml2-language-server.cmd", "sysml2-language-server.exe",
		"sysml2-lsp", "sysml2-lsp.cmd", "sysml2-lsp.exe",
		"sysml-language-server", "sysml-language-server.cmd", "sysml-language-server.exe":
		return true
	default:
		return strings.Contains(base, "kerml-language-server") ||
			strings.Contains(base, "kerml-lsp") ||
			strings.Contains(base, "sysml2-language-server") ||
...
func referencesKerMLServer(args []string) bool {
	return slices.ContainsFunc(args, func(arg string) bool {
		normalized := strings.ToLower(strings.TrimSpace(arg))
		normalized = strings.ReplaceAll(normalized, "\\", "/")
		return strings.Contains(normalized, "kerml-language-server") ||
			strings.Contains(normalized, "kerml-lsp") ||
			strings.Contains(normalized, "sysml2-language-server") ||
	...
	return map[string]any{
		...
		"properties": map[string]any{
			"command": map[string]any{
				"type":        "string",
				"description": "KerML language-server command name to execute (see kerml_list_extension_commands).",
			},
