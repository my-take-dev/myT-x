// Package lean4 は Lean4 向けの MCP 拡張ツールを提供する。
package lean4
...
const lean4ReferenceURL = "https://github.com/leanprover/lean4"

// BuildTools は Lean4 言語サーバー向けの拡張ツールを構築する。
func BuildTools(client *lsp.Client, rootDir string) []mcp.Tool {
	...
	return []mcp.Tool{
		{
			Name:        "lean4_list_extension_commands",
			Description: triadDescription("Inspect workspace commands advertised via executeCommandProvider.commands by the connected Lean4 language server", "none", "read"),
			InputSchema: emptySchema(),
			Handler:     svc.handleListCommands,
		},
		{
			Name:        "lean4_execute_extension_command",
			Description: triadDescription("Run one workspace command advertised by the connected Lean4 language server", "command, arguments?", "exec"),
...
// Matches は設定されたコマンド/引数が Lean4 言語サーバーを示す場合に true を返す。
func Matches(command string, args []string) bool {
	if looksLikeLean4Server(command) {
		return true
	}

	if slices.ContainsFunc(args, looksLikeLean4Server) {
		return true
	}
...
func (s *service) handleListCommands(_ context.Context, _ map[string]any) (any, error) {
	...
	for _, name := range commands {
		when, argsHint, effect := capabilityCommandGuide(name, "Lean4")
	...
	return map[string]any{
		...
		"count":             len(commands),
		"references":        []string{lean4ReferenceURL},
	}, nil
}
...
func (s *service) handleExecuteCommand(ctx context.Context, args map[string]any) (any, error) {
	...
	when, argsHint, effect := capabilityCommandGuide(command, "Lean4")
...
func looksLikeLean4Server(value string) bool {
	base := strings.ToLower(strings.TrimSpace(filepath.Base(value)))
	switch base {
	case "lean4", "lean4.exe", "lean4.cmd", "lean4.bat",
		"lean4-language-server", "lean4-language-server.exe",
		"lean-language-server", "lean-language-server.exe":
		return true
	default:
		return strings.Contains(base, "lean4") || strings.Contains(base, "lean-language-server")
	}
}
	...
	return map[string]any{
		...
		"properties": map[string]any{
			"command": map[string]any{
				"type":        "string",
				"description": "Lean4 language-server command name to execute (see lean4_list_extension_commands).",
			},
