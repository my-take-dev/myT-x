// Package lox は Lox 向けの MCP 拡張ツールを提供する。
package lox
...
const loxReferenceURL = "https://github.com/ajeetdsouza/loxcraft"

// BuildTools は Lox 言語サーバー向けの拡張ツールを構築する。
func BuildTools(client *lsp.Client, rootDir string) []mcp.Tool {
	...
	return []mcp.Tool{
		{
			Name:        "lox_list_extension_commands",
			Description: triadDescription("Inspect workspace commands advertised via executeCommandProvider.commands by the connected Lox language server", "none", "read"),
			InputSchema: emptySchema(),
			Handler:     svc.handleListCommands,
		},
		{
			Name:        "lox_execute_extension_command",
			Description: triadDescription("Run one workspace command advertised by the connected Lox language server", "command, arguments?", "exec"),
...
// Matches は設定されたコマンド/引数が Lox 言語サーバーを示す場合に true を返す。
func Matches(command string, args []string) bool {
	if looksLikeLoxServer(command) {
		return true
	}

	if slices.ContainsFunc(args, looksLikeLoxServer) {
		return true
	}
...
func (s *service) handleListCommands(_ context.Context, _ map[string]any) (any, error) {
	...
	for _, name := range commands {
		when, argsHint, effect := capabilityCommandGuide(name, "Lox")
	...
	return map[string]any{
		...
		"count":             len(commands),
		"references":        []string{loxReferenceURL},
	}, nil
}
...
func (s *service) handleExecuteCommand(ctx context.Context, args map[string]any) (any, error) {
	...
	when, argsHint, effect := capabilityCommandGuide(command, "Lox")
...
func looksLikeLoxServer(value string) bool {
	base := strings.ToLower(strings.TrimSpace(filepath.Base(value)))
	switch base {
	case "loxcraft", "loxcraft.exe",
		"loxcraft-lsp", "loxcraft-lsp.exe",
		"lox-language-server", "lox-language-server.exe":
		return true
	default:
		return strings.Contains(base, "loxcraft") || strings.Contains(base, "lox-language-server")
	}
}
	...
	return map[string]any{
		...
		"properties": map[string]any{
			"command": map[string]any{
				"type":        "string",
				"description": "Lox language-server command name to execute (see lox_list_extension_commands).",
			},
