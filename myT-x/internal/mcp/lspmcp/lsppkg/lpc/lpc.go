// Package lpc は LPC 向けの MCP 拡張ツールを提供する。
package lpc
...
const lpcReferenceURL = "https://github.com/jchmura/lpc-language-server"

// BuildTools は LPC 言語サーバー向けの拡張ツールを構築する。
func BuildTools(client *lsp.Client, rootDir string) []mcp.Tool {
	...
	return []mcp.Tool{
		{
			Name:        "lpc_list_extension_commands",
			Description: triadDescription("Inspect workspace commands advertised via executeCommandProvider.commands by the connected LPC language server", "none", "read"),
			InputSchema: emptySchema(),
			Handler:     svc.handleListCommands,
		},
		{
			Name:        "lpc_execute_extension_command",
			Description: triadDescription("Run one workspace command advertised by the connected LPC language server", "command, arguments?", "exec"),
...
// Matches は設定されたコマンド/引数が LPC 言語サーバーを示す場合に true を返す。
func Matches(command string, args []string) bool {
	if looksLikeLPCServer(command) {
		return true
	}

	if slices.ContainsFunc(args, looksLikeLPCServer) {
		return true
	}

	if looksLikeNode(command) && referencesLPCServer(args) {
		return true
	}
...
func (s *service) handleListCommands(_ context.Context, _ map[string]any) (any, error) {
	...
	for _, name := range commands {
		when, argsHint, effect := capabilityCommandGuide(name, "LPC")
	...
	return map[string]any{
		...
		"count":             len(commands),
		"references":        []string{lpcReferenceURL},
	}, nil
}
...
func (s *service) handleExecuteCommand(ctx context.Context, args map[string]any) (any, error) {
	...
	when, argsHint, effect := capabilityCommandGuide(command, "LPC")
...
func looksLikeLPCServer(value string) bool {
	base := strings.ToLower(strings.TrimSpace(filepath.Base(value)))
	switch base {
	case "lpc-language-server", "lpc-language-server.exe", "lpc-language-server.cmd", "lpc-language-server.bat":
		return true
	default:
		return strings.Contains(base, "lpc-language-server")
	}
}
...
func referencesLPCServer(args []string) bool {
	return slices.ContainsFunc(args, func(arg string) bool {
		normalized := strings.ToLower(strings.TrimSpace(arg))
		return strings.Contains(normalized, "lpc-language-server")
	})
}
	...
	return map[string]any{
		...
		"properties": map[string]any{
			"command": map[string]any{
				"type":        "string",
				"description": "LPC language-server command name to execute (see lpc_list_extension_commands).",
			},
