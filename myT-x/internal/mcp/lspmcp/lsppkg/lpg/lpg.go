// Package lpg は LPG 向けの MCP 拡張ツールを提供する。
package lpg
...
const lpgReferenceURL = "https://github.com/kuafuwang/LPG-language-server"

// BuildTools は LPG 言語サーバー向けの拡張ツールを構築する。
func BuildTools(client *lsp.Client, rootDir string) []mcp.Tool {
	...
	return []mcp.Tool{
		{
			Name:        "lpg_list_extension_commands",
			Description: triadDescription("Inspect workspace commands advertised via executeCommandProvider.commands by the connected LPG language server", "none", "read"),
			InputSchema: emptySchema(),
			Handler:     svc.handleListCommands,
		},
		{
			Name:        "lpg_execute_extension_command",
			Description: triadDescription("Run one workspace command advertised by the connected LPG language server", "command, arguments?", "exec"),
...
// Matches は設定されたコマンド/引数が LPG 言語サーバーを示す場合に true を返す。
func Matches(command string, args []string) bool {
	if looksLikeLPGServer(command) {
		return true
	}

	if slices.ContainsFunc(args, looksLikeLPGServer) {
		return true
	}
...
func (s *service) handleListCommands(_ context.Context, _ map[string]any) (any, error) {
	...
	for _, name := range commands {
		when, argsHint, effect := capabilityCommandGuide(name, "LPG")
	...
	return map[string]any{
		...
		"count":             len(commands),
		"references":        []string{lpgReferenceURL},
	}, nil
}
...
func (s *service) handleExecuteCommand(ctx context.Context, args map[string]any) (any, error) {
	...
	when, argsHint, effect := capabilityCommandGuide(command, "LPG")
...
func looksLikeLPGServer(value string) bool {
	normalized := strings.ToLower(strings.TrimSpace(value))
	base := strings.ToLower(strings.TrimSpace(filepath.Base(value)))
	switch base {
	case "lpg-language-server", "lpg-language-server.exe", "lpg-language-server.cmd",
		"lpg_language_server", "lpg_language_server.exe":
		return true
	default:
		return strings.Contains(base, "lpg-language-server") ||
			strings.Contains(base, "lpg_language_server") ||
			strings.Contains(normalized, "lpg-language-server") ||
			strings.Contains(normalized, "lpg_language_server")
	}
}
	...
	return map[string]any{
		...
		"properties": map[string]any{
			"command": map[string]any{
				"type":        "string",
				"description": "LPG language-server command name to execute (see lpg_list_extension_commands).",
			},
