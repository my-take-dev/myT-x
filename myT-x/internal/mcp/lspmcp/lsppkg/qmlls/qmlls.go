// Package qmlls は QML 向けの MCP 拡張ツールを提供する。
package qmlls
...
// BuildTools は QML 言語サーバー向けの拡張ツールを構築する。
func BuildTools(client *lsp.Client, rootDir string) []mcp.Tool {
	...
	return []mcp.Tool{
		{
			Name:        "qmlls_list_extension_commands",
			Description: triadDescription("Inspect workspace commands advertised via executeCommandProvider.commands by the connected QML language server", "none", "read"),
			InputSchema: emptySchema(),
			Handler:     svc.handleListCommands,
		},
		{
			Name:        "qmlls_execute_extension_command",
			Description: triadDescription("Run one workspace command advertised by the connected QML language server", "command, arguments?", "exec"),
...
// Matches は設定されたコマンド/引数が QML 言語サーバーを示す場合に true を返す。
func Matches(command string, args []string) bool {
	if looksLikeQmlls(command) {
		return true
	}

	return slices.ContainsFunc(args, looksLikeQmlls)
}
...
func (s *service) handleListCommands(_ context.Context, _ map[string]any) (any, error) {
	...
	for _, name := range commands {
		when := describeCapabilityWhen(name, "QML")
		argsHint := describeCapabilityArgs(name, "QML")
		effect := "exec"
		out = append(out, map[string]any{
			...
			"args":        argsHint,
			"effect":      effect,
			"description": describeCapabilityCommand(name, "QML"),
			"available":   true,
...
func (s *service) handleExecuteCommand(ctx context.Context, args map[string]any) (any, error) {
	...
	when := describeCapabilityWhen(command, "QML")
	argsHint := describeCapabilityArgs(command, "QML")
	effect := "exec"
	...
	return map[string]any{
		...
		"root":              s.rootDir,
		"command":           command,
		"commandGuide":      map[string]any{"when": when, "args": argsHint, "effect": effect, "description": describeCapabilityCommand(command, "QML")},
		"arguments":         arguments,
		"result":            result,
...
func looksLikeQmlls(value string) bool {
	base := strings.ToLower(strings.TrimSpace(filepath.Base(value)))
	return base == "qmlls" || base == "qmlls.exe"
}
	...
	return map[string]any{
		...
		"properties": map[string]any{
			"command": map[string]any{
				"type":        "string",
				"description": "qmlls command name to execute (see qmlls_list_extension_commands).",
			},
