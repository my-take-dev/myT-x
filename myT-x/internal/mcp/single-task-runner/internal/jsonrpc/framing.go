...
// Framing represents the stdio framing format.
type Framing int
...
// ReadMessage reads one JSON-RPC message using Content-Length or line-delimited framing.
func ReadMessage(reader *bufio.Reader) ([]byte, error) {
	payload, _, err := ReadMessageWithFraming(reader)
	return payload, err
}

// ReadMessageWithFraming reads one JSON-RPC message and returns the detected framing.
...
// WriteMessage writes one framed JSON-RPC message using Content-Length framing.
...
// WriteJSON writes one framed JSON value using Content-Length framing.
...
// WriteJSONLine writes one line-delimited JSON value.
