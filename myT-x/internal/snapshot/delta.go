...
// Field-by-field comparators replace reflect.DeepEqual on the hot path.
...
func (s *Service) snapshotDelta(snapshots []tmux.SessionSnapshot) (tmux.SessionSnapshotDelta, bool, bool) {
	s.snapshotDeltaMu.Lock()
	defer s.snapshotDeltaMu.Unlock()
	...
	if !s.snapshotPrimed {
		if s.snapshotCache == nil {
			s.snapshotCache = make(map[string]tmux.SessionSnapshot, len(snapshots))
		}
		for _, snapshot := range snapshots {
			s.snapshotCache[snapshot.Name] = snapshot
		}
	...
	previous := copySnapshotCache(s.snapshotCache)
	s.snapshotMu.Unlock()
	...
	s.snapshotMu.Lock()
	if !s.snapshotPrimed || s.snapshotCache == nil {
		s.snapshotCache = make(map[string]tmux.SessionSnapshot, len(snapshots))
		for _, snapshot := range snapshots {
			s.snapshotCache[snapshot.Name] = snapshot
		}
